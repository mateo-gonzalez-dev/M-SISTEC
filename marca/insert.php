...
<body>
    ...
    <div id="contenido">
        ...
        if(isset($_POST['enviar'])) {
            $nom = $_POST['txtnombre'];
            if(!empty($nom)) {
                $verificar = mysqli_query($conexion,"SELECT COUNT(*)AS total FROM marca WHERE nombre_marca='$nom'");

                $existe = mysqli_fetch_assoc($verificar);

                if($existe['total']>0){
                    echo "<script>alert(' La marca ya existe en la base de datos.');</script>";
                }else{
                    $sql = "INSERT INTO marca (nombre_marca) VALUES ('$nom')";

                    $result = mysqli_query($conexion, $sql) or die (mysqli_error($conexion));

                    echo "<script>alert('✅ Marca registrado exitosamente.');</script>";
