...
<body>
    ...
    <div id="contenido">
        ...
        if(isset($_POST['enviar'])) {
            $nom = $_POST['txtnombre'];
            if(!empty($nom)) {
                $verificar = mysqli_query($conexion,"SELECT COUNT(*)AS total FROM pais WHERE nombre='$nom'");

                $existe = mysqli_fetch_assoc($verificar);

                if($existe['total']>0){
                    echo "<script>alert('El pais ya existe en la base de datos.');</script>";
                }else{
                    $sql = "INSERT INTO pais (nombre) VALUES ('$nom')";

                    $result = mysqli_query($conexion, $sql) or die (mysqli_error($conexion));

                    echo "<script>alert('✅ Pais registrado exitosamente.');</script>";
