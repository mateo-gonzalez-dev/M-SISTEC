    ...
    <div id="contenido">
        <form action="select.php" method="POST" name="select">
            <fieldset>
                ...
                <?php
                $sql_registros = mysqli_query($conexion,'SELECT COUNT(*) AS total FROM categoria');
                ...
                $consulta = "SELECT * FROM categoria LIMIT $desde,$por_pagina";

                $ejecutar = mysqli_query($conexion, $consulta);

                if(mysqli_num_rows($ejecutar)>0){
                    while($fila = mysqli_fetch_assoc($ejecutar)){
                        $id_tipo_categoria = $fila['id_categoria'];
                        $nom = $fila['tipo_categoria'];

                        echo "
                    <tr>
                      <td>$id_tipo_categoria</td>
                      <td>$nom</td>
                      <td>
                        <a class='btn-accion btn-actualizar' href='update.php?ACTUALIZAR=$id_tipo_categoria' title='Actualizar'>
                          <i class='fa-solid fa-pen-to-square'></i>
                        </a>
                      </td>
                      <td>
                        <a class='btn-accion btn-eliminar' href='select.php?ELIMINAR=$id_tipo_categoria' title='Eliminar' onclick=\"return confirm('¿Desea eliminar este rol?');\">
        ...
        if(isset($_GET["ELIMINAR"])){
            ...
            $eliminar = "DELETE FROM categoria WHERE id_categoria = '$borrar_id'";
