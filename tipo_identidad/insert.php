...
<head>
    ...
    <title>T Identidad - INSERT</title>
...
<body>
    ...
    <div id="contenido">
        <form action="insert.php" method="POST" name="insert">
            <fieldset>
                <legend id="titulo-form">
                    <h2>Insertar Tipo de Identidad</h2>
        ...
        if(isset($_POST['enviar'])) {
            $nom = $_POST['txtnombre'];
            if(!empty($nom)) {
                $verificar = mysqli_query($conexion,"SELECT COUNT(*)AS total FROM tipo_identidad WHERE tipo_identidad='$nom'");

                $existe = mysqli_fetch_assoc($verificar);

                if($existe['total']>0){
                    echo "<script>alert(' El Tipo de Identidad ya existe en la base de datos.');</script>";
                }else{
                    $sql = "INSERT INTO tipo_identidad (tipo_identidad) VALUES ('$nom')";

                    $result = mysqli_query($conexion, $sql) or die (mysqli_error($conexion));

                    echo "<script>alert('✅ Tipo de Identidad registrado exitosamente.');</script>";
